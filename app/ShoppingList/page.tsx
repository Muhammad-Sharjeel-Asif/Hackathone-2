...
const initialCartItems = [
    { name: "Burger", price: 10.99, quantity: 2, image: Pic01 },
    { name: "Fresh Lime", price: 3.49, quantity: 1, image: Pic02 },
    { name: "Pizza", price: 9.99, quantity: 4, image: Pic03 },
    { name: "Chocolate Muffin", price: 4.49, quantity: 1, image: Pic04 },
    { name: "Cheese Butter", price: 11.99, quantity: 3, image: Pic05 },
];
                    ...
                    <tbody className='text-[#333333]'>
                        {initialCartItems.map((item, index) => (
                            <tr key={index} className="border-b">
                                ...
                                <td className="p-4">
                                    <div className='border rounded-[18px] px-2 py-1 text-center"
                                        min="0" h-[32.04px] max-w-[110.17px] flex justify-evenly'>
                                        <button className='text-[#E0E0E0]'><Image src={Minus} alt='Plus' /></button>
                                        <input
                                            type="number"
                                            value={item.quantity}
                                            className="w-16 pl-2 text-center"
                                        />
                                        <button className='text-[#1F242E]'><Image src={Plus} alt='Plus' /></button>
                                    </div>
                                </td>
                                <td className="p-4">${(item.price * item.quantity).toFixed(2)}</td>
                                <td className="px-10 cursor-pointer hover:text-[#FF9F0D]"> &times;
                                </td>
                            </tr>
                        ))}
                    </tbody>
